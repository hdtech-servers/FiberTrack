...
def order_detail(request, order_number):
    ...
    for item in order.items.all():
        subtotal = item.buying_price * item.quantity_ordered
        items.append({
...
def create_order(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and items_data:
            # Save the order with payment option and amount paid
            order = form.save(commit=False)
            order.payment_option = form.cleaned_data['payment_option']
            order.paid_amount = form.cleaned_data['paid_amount'] or 0.00
            order.save()
            ...
            order.save()  # Save again to update payment status
...
def receive_stock(request, order_number):
    ...
    if request.method == 'POST':
        ...
        # Update payment status if payment is completed during stock reception
        if order.payment_option == 'pay_on_delivery':
            order.paid_amount = float(request.POST.get('paid_amount', 0))
            order.save()  # Save to update payment status
...
def generate_lpo_pdf(request, order_number):
    ...
    shop = OrganizationSettings.objects.first()
